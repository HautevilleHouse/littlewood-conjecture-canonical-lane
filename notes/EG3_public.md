...
In-paper anchor: `paper/LITTLEWOOD_CONJECTURE_PREPRINT.md` (`LC_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `prove Littlewood's conjecture by routing admissible homogeneous-dynamics states through flow coercivity, nonescape capture, compactness, rigidity, exceptional-set transfer, and strict coherence`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`LC_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and escape windows do not collapse.
This is the first-failure capture contribution to `M_LC`.
...
If near-failure extraction and no-Zeno spacing hold, then `LC_G3` prevents hidden drift before rigidity is applied.
