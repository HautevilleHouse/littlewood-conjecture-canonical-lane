...
In-paper anchor: `paper/LITTLEWOOD_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `prove Littlewood's conjecture by routing admissible homogeneous-dynamics states through flow coercivity, nonescape capture, compactness, rigidity, exceptional-set transfer, and strict coherence`.
...
- endpoint lock: `exceptional_transfer`.
...
2. the endpoint-transfer lock `exceptional_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_LC` remains positive.
...
Payload: verify `exceptional_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_LC` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `exceptional_transfer`
